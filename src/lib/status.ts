...
export function classifyTrackingStatus(status: string): 'delivered' | 'returned' | 'transit' | 'delivery' | 'others' {
  ...
  if (['en livraison', 'en cours de livraison', 'out for delivery', 'قيد التوزيع', 'توزيع'].includes(s)) return 'delivery';
  if (['en transit', 'transit', 'في الطريق', 'expédié', 'en route', 'ramassé', 'colis ramassé'].includes(s)) return 'transit';
  return 'others';
}
